...
| `outcome` | intended item | `type`, `dimension`, `subject`, `standard`, `dok`, `focus`, `mode` (inference/conclusion/author-intent), `other`, `stem`, `stem-b`, `rubric` |
| `band` | rubric row | `score`, `descriptor` (inside an outcome's `rubric`) |
