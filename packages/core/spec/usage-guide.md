...
In scope: SBAC Grade 5 · Claim 1 for targets **T4** (Reasoning & Evidence, literary, RL), **T11** (Reasoning & Evidence, informational, RI), and **T9** (Central Ideas, informational, RI-1/RI-2); a single passage; a content graph of supported and distractor claims plus evidence sources tagged by role; the per-target dimensions and distractor taxonomy; item types `ebsr`, `hot-text`, `short-text`, `multiple-choice`, `multi-select`; DOK r-dok1..r-dok3 (R&E r-dok3; T9 r-dok2/r-dok3). Out of scope: other claims/grades or targets beyond T4/T11/T9 (T8 & T10 are planned); single-part Hot-Text for T9; dual-text stimuli; compile-time LLM generation; auto-scoring of short text; cross-language composition.
...
- **Target** — `target c1-t4` (literary, Reasoning & Evidence), `target c1-t11` (informational, Reasoning & Evidence), or `target c1-t9` (informational, Central Ideas); required, first top-level form. Selects the dimensions, standards, distractor taxonomy, DOK, and stem catalog.
...
- **Item types** — `ebsr` (two-part: statement → evidence), `hot-text` (statement → click sentences), `short-text` (constructed response), `multiple-choice` (one correct, single-part), `multi-select` (exact correct set, single-part; `focus` is a list). MC/Multi-Select have no Part B.
