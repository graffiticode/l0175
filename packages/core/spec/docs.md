...
`c1-t1` (literary, RL-1), `c1-t8` (informational, RI-1/RI-7), or `c1-t10` (informational, RI-4/L-4). It is
...
| ---- | :---: | ------- | ----------- |
| **target** | 2 | `target c1-t11` | Required, top level; selects the learning-target profile (`c1-t4` / `c1-t11` / `c1-t9` / `c1-t1` / `c1-t8` / `c1-t10`) |
...
- **top level** — `target` (required: `c1-t4` | `c1-t11` | `c1-t9` | `c1-t1` | `c1-t8` | `c1-t10`), `title` (optional), `grade` (optional reading-level target; defaults to the target's grade), `words` (c1-t10 only — a list of `word`s)
...
- `target`: `c1-t4` (literary, R&E), `c1-t11` (informational, R&E), `c1-t9` (informational, Central Ideas), `c1-t1` (**literary**, Key Details), `c1-t8` (informational, Key Details), `c1-t10` (informational, Word Meanings)
...
- `standard` — full CCSS Grade-5 strand for the text type accepted; the dimension's companion is inferred when omitted. Primary companions — (c1-t4): `rl-1` + `rl-2` (theme/topic) / `rl-3` / `rl-6` [full RL `rl-1`–`rl-7`/`rl-9`, no `rl-8`] · (c1-t11): `ri-1` + `ri-3` / `ri-6` / `ri-7` / `ri-8` · (c1-t9): `ri-1` + `ri-2` · (c1-t1): `rl-1` alone (no companion) · (c1-t8): `ri-1` + `ri-7` [c1-t11/t9/t8 accept full RI `ri-1`–`ri-9`] · (c1-t10): `ri-4` + the `l-4` / `l-5` families
